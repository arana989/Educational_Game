...
function dragEnd() {
    ...
    let pieces = document.getElementById("board").getElementsByTagName("*")
    let startCounter = 1;
    let mazeSolved = true;
    for (let i = 0, max = pieces.length; i < max; i++) {
        // Do something with the element here
        let fileName = getFileName(pieces[i].src);
        let file = fileName.replace(/\.[^/.]+$/, "")
        console.log(file);
        if (startCounter == parseInt(file)) { startCounter += 1; }
        else { mazeSolved = false; }
    }
    if (mazeSolved) { puzzleSolved(); }
}
getImages();
function getImages() {
    for (let r = 0; r < rows; r++) {
...
function puzzleSolved() {
    ...
    document.getElementById('output').innerHTML = 'puzzle is solved...';
